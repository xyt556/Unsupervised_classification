...
def class_editor_ui():
    ...
    # 处理配置导入
    if st.session_state.config_imported:
        st.success("✓ 配置已成功导入！")
        st.session_state.config_imported = False
        st.rerun()
    ...
    with col1:
        ...
        for i, (col, template_key) in enumerate(zip(cols, template_keys[:5])):
            with col:
                ...
                if st.button(template_info['name'], key=f"template_{i}", use_container_width=True):
    ...
    with col2:
        ...
        with st.form("import_config_form"):
            uploaded_config = st.file_uploader("📤 导入配置文件", type=['json'], key='config_upload')
            submit_import = st.form_submit_button("✅ 导入配置", use_container_width=True)
            
            if submit_import and uploaded_config:
                try:
                    config = json.load(uploaded_config)
                    update_class_config_from_import(config)
                    st.success("✓ 配置导入成功！")
    ...
    with st.form("class_editor_form"):
        updated_config = st.session_state.class_config.copy()
        
        for class_id in st.session_state.unique_classes:
            class_id = int(class_id)
            class_info = updated_config[class_id]
            
            with st.expander(f"✏️ {class_info['name']} (编号: {class_id})", 
                            expanded=False):
                ...
                with col2:
                    ...
                    new_color = st.color_picker(
                        "显示颜色",
                        value=class_info['color'],
                        key=f"color_{class_id}",
                        help="选择此类别在地图上显示的颜色"
                    )
                    class_info['color'] = new_color
                ...
                with col3:
                    ...
                    if template_choice != '手动设置' and template_choice in LANDCOVER_TEMPLATES:
                        if st.button("应用此模板", key=f"apply_{class_id}", use_container_width=True):
                            ...
                            class_info.update(template_info)
                            st.success(f"✓ 已应用模板: {template_info['name']}")
                
                # 描述
                new_desc = st.text_area(
                    "类别描述（可选）",
                    value=class_info.get('description', ''),
                    key=f"desc_{class_id}",
                    height=60,
                    help="添加此类别的详细描述"
                )
                ...
                # 预览
                st.markdown(f"""
                <div style='background-color: {class_info["color"]}; 
                            padding: 15px; border-radius: 5px; margin-top: 10px; 
                            border: 2px solid #333; color: white; text-shadow: 1px 1px 2px black;
                            font-size: 16px; font-weight: bold; text-align: center;'>
                    预览效果: {class_info["name"]}
                </div>
                """, unsafe_allow_html=True)
            
            updated_config[class_id] = class_info
        
        # 应用更改按钮
        submitted = st.form_submit_button("✅ 保存所有更改", type="primary", use_container_width=True)
        if submitted:
            st.session_state.class_config = updated_config
            st.success("✓ 所有更改已保存！图表将自动更新")
            st.rerun()
